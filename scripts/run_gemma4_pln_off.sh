#!/bin/bash
# After all other PLN-off runs finish, run Gemma 4 E4B mapped-903 with
# thinking ON via chat_template_kwargs (set by CTI_RAG_LLM_ENABLE_THINKING=1).

set -uo pipefail
LOG_DIR=logs/reeval_no_prefer_last_nvd
mkdir -p "$LOG_DIR"

ORCH_PID=${ORCH_PID:-2685780}
FOLLOWUP_PID=${FOLLOWUP_PID:-2685781}

echo "[$(date -Iseconds)] Waiting for orchestrator PID $ORCH_PID and follow-up PID $FOLLOWUP_PID to finish..."
while kill -0 "$ORCH_PID" 2>/dev/null || kill -0 "$FOLLOWUP_PID" 2>/dev/null; do
  sleep 60
done
echo "[$(date -Iseconds)] Prereqs done. Starting Gemma 4 E4B (thinking ON, PLN off)."
...
serve_a="$LOG_DIR/vllm_gemma4_gpu0_${stamp}.log"
serve_b="$LOG_DIR/vllm_gemma4_gpu1_${stamp}.log"
eval_log="$LOG_DIR/eval_gemma4_no_pln_${stamp}.log"
fail_dst="$LOG_DIR/eval_failures_gemma4_${stamp}.jsonl"
timing_dst="$LOG_DIR/eval_timing_gemma4_${stamp}.jsonl"

CUDA_VISIBLE_DEVICES=0 nohup /home/sheng/miniconda3/bin/vllm serve google/gemma-4-E4B-it \
  --served-model-name "Qwen/Qwen2.5-7B-Instruct" \
  --max-model-len 24576 --gpu-memory-utilization 0.80 \
  --enable-prefix-caching --trust-remote-code --port 8000 > "$serve_a" 2>&1 &
disown
CUDA_VISIBLE_DEVICES=1 nohup /home/sheng/miniconda3/bin/vllm serve google/gemma-4-E4B-it \
  --served-model-name "Qwen/Qwen2.5-7B-Instruct" \
  --max-model-len 24576 --gpu-memory-utilization 0.80 \
...
echo "[$(date -Iseconds)] Gemma 4 PLN-off mapped-903 complete." | tee -a "$eval_log"
